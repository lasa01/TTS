...
from TTS.tts.layers.common_layers import Prenet
from TTS.tts.layers.attentions import Linear
from TTS.tts.layers.tacotron2 import ConvBNBlock
from .common_layers import init_attn
...
class Decoder(nn.Module):
    ...
    frame_channels: Final[int]
    r_init: Final[int]
    r: Final[int]
    encoder_embedding_dim: Final[int]
    separate_stopnet: Final[bool]
    max_decoder_steps: Final[int]
    stop_threshold: Final[float]
    query_dim: Final[int]
    decoder_rnn_dim: Final[int]
    prenet_dim: Final[int]
    attn_dim: Final[int]
    p_attention_dropout: Final[float]
    p_decoder_dropout: Final[float]
    ...
    def __init__(self, in_channels, frame_channels, r, attn_type, attn_win, attn_norm,
        ...
        self.attention_rnn = nn.LSTM(self.prenet_dim + in_channels,
                                     self.query_dim,
                                     num_layers=1,
                                     bias=True)
        ...
        self.decoder_rnn = nn.LSTM(self.query_dim + in_channels,
        ...
        self._init_states(torch.zeros(1, 1, in_channels))
    ...
    def _parse_outputs(self, outputs: List[Tensor], stop_tokens: List[Tensor], alignments: List[Tensor]):
    ...
    def decode(self, memory):
        ...
        attention_rnn_result = self.attention_rnn(
            query_input.unsqueeze(0), (self.query.unsqueeze(0), self.attention_rnn_cell_state.unsqueeze(0)))
        self.query = attention_rnn_result[1][0].squeeze(0)
        self.attention_rnn_cell_state = attention_rnn_result[1][1].squeeze(0)
        ...
        decoder_rnn_result = self.decoder_rnn(
            decoder_rnn_input.unsqueeze(0), (self.decoder_hidden.unsqueeze(0), self.decoder_cell.unsqueeze(0)))
        self.decoder_hidden = decoder_rnn_result[1][0].squeeze(0)
        self.decoder_cell = decoder_rnn_result[1][1].squeeze(0)
